...
setwd("~/Code/R/book-exercises/chapter-12-exercises/exercise-1")

# Load the `data/avocado.csv` file into a variable `avocados`
# Make sure strings are *not* read in as factors
wb_data <- read.csv( "data/avocado.csv", stringsAsFactors = F)
...
wb_data <- wb_data %>% 
  +     mutate(Date = as.Date(Date)) # needed hint

# The file had some uninformative column names, so rename these columns:
# `X4046` to `small_haas`
# `X4225` to `large_haas`
# `X4770` to `xlarge_haas`
rename(wb_data, rename,`X4046` = `small_haas`)
rename(wb_data, rename,`X4225` = `large_haas`)
rename(wb_data, rename, `X4770` = `xlarge_haas`)
